...
>
> **Since v0.20.0 the host support floor is DSH `0.1.6-alpha.2`**: that DSH version ships its own right-Sidebar **terminal** and **browser**, so the plugin **deleted its own terminal** (the whole xterm + node-pty stack) and **handed the browser view to the built-in**, keeping only the per-protocol link takeover. Hosts on 0.1.5-rc.* or earlier should stay on **v0.19.1**.
...
<div align="center">
  ...
  <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (v0.20.0): 0.1.6-alpha.2+" src="https://img.shields.io/badge/DSH-0.1.6--alpha.2%2B-4d6bfe" /></a>
...
- **🌐 Built-in browser + link routing**: web tabs are provided by DSH's own `ui-sidebar-browser` (multiple tabs, back / forward / reload, sandboxed iframe); the plugin keeps the **external-link takeover** in the chat and UI — HTTP opens in the sidebar, HTTPS goes to the system browser (both adjustable in the settings page), the protocol-based split the host itself does not provide
- **💻 Terminal (provided by DSH)**: the right-Sidebar terminal is provided by DSH's own `ui-sidebar-terminal` (shell picker, rename, reconnect, restore after reload, theme following). The plugin no longer ships xterm + node-pty
...
> 📌 **Stable release**: starting with `v0.20.0` the plugin targets DSH **0.1.6-alpha.2+** (npm dist-tag `latest`; peer floor `^0.1.6-alpha.2`). **The floor had to move**: semver's prerelease rule means `^0.1.5-rc.1` can never match any `0.1.6` prerelease. Hosts still on DSH 0.1.5-rc.* (including the 0.1.5-rc.2 that is npm's `latest`) should pin **`dsh-better-sidebar@0.19.1`** — 0.1.6 carries two genuinely breaking contracts and this version writes no runtime compatibility layer; DSH 0.1.5-alpha.2 and earlier should likewise stay on their old versions (`0.19.0-alpha.1` / `0.18.x` / `0.17.1`).

```sh
dsh plugin --profile web add dsh-better-sidebar@latest
```

> Since v0.20.0 the plugin depends on no package that needs a build script (`node-pty` left with the plugin's own terminal), so installing is **one step** — the `pnpm approve-builds` re-run is no longer part of it. DSH 0.1.6 also ships a Web **Plugins page** (`ui-plugin-manager`) plus the `plugin_manager` tool, so you can enable / disable it right there once installed.
...
1. Run: dsh plugin --profile web add dsh-better-sidebar@latest (one step since v0.20.0 — no build-script approval needed)
...
| Symptom | Cause & fix |
|---|---|
| `Ignored build scripts` | pnpm 11 blocked a **transitive** dependency's build script. Run `pnpm approve-builds` (no `--all`) in the profile directory (`~/.dsh/profiles/web`) and follow its prompts — since v0.20.0 the plugin itself has no build-script dependency. |
...
| Terminal unusable / shell fails to start | The terminal and `node-pty` now both come from **DSH's own `ui-sidebar-terminal`** (this plugin no longer ships one since v0.20.0). Consult DSH's own docs for problems; if the error mentions build scripts, see the row above. |
...
**Supported DSH versions**: <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (v0.20.0): 0.1.6-alpha.2+" src="https://img.shields.io/badge/DSH-0.1.6--alpha.2%2B-4d6bfe" /></a> · full release history on the [Releases](https://github.com/omdsh-dev/DSH-better-sidebar/releases) page

### v0.20.0

> 📌 **Stable release** (npm `latest`, no prerelease suffix): supports **DSH 0.1.6-alpha.2+** only (peer floor `^0.1.6-alpha.2`, CI pins `@deepseek-ai/dsh@0.1.6-alpha.2`). **This is a breaking-change release**: hosts on 0.1.5-rc.* should pin `dsh-better-sidebar@0.19.1`. See [docs/plans/2026-09-21-dsh-0.1.6-alpha.2-adaptation.md](docs/plans/2026-09-21-dsh-0.1.6-alpha.2-adaptation.md) for the adaptation record.
...
Windows / Linux / macOS (macOS validated daily; the rest covered by unit tests). Since v0.20.0 the plugin carries no native dependencies (the terminal and `node-pty` went back to DSH itself), so building needs only Node + pnpm, with no compiler toolchain.
